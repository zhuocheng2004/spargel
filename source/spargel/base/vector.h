...
namespace spargel::base {
    ...
    namespace __vector {
        ...
        public:
            ...
            vector(T const* begin, T const* end) { init_with_copy(begin, end); }

            /// @brief copy from another vector
            vector(vector const& other) { init_with_copy(other._begin, other._end); }
            ...
            void set_count(ssize cnt) { _end = _begin + cnt; }
        ...
        private:
            ...
            void grow_to(ssize req) {
                ...
                T* new_begin = static_cast<T*>(DefaultAllocator{}.allocate(sizeof(T) * new_cap));
                if (_begin != nullptr) {
            ...
            void allocate(ssize count) {
                ASSERT(count > 0);
                _begin = static_cast<T*>(DefaultAllocator{}.allocate(sizeof(T) * count));
                _end = _begin;
                _capacity = _begin + count;
            }

            void deallocate() {
                DefaultAllocator{}.deallocate(_begin, sizeof(T) * capacity());
                _begin = nullptr;
                _end = nullptr;
                _capacity = nullptr;
            }
