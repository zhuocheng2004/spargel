...
#include <spargel/base/execution/sender.h>
...
#include <spargel/base/meta/is_assignable.h>
#include <spargel/base/meta/is_base.h>
#include <spargel/base/meta/is_constructible.h>
#include <spargel/base/meta/is_convertible.h>
#include <spargel/base/meta/is_destructible.h>
#include <spargel/base/meta/is_same.h>
...
static_assert(is_same<int const, add_const<int>>);

static_assert(is_same<int&, add_lvalue_reference<int>>);
static_assert(is_same<int&, add_lvalue_reference<int&>>);
static_assert(is_same<int&, add_lvalue_reference<int&&>>);

static_assert(is_same<int&&, add_rvalue_reference<int>>);
static_assert(is_same<int&, add_rvalue_reference<int&>>);
static_assert(is_same<int&&, add_rvalue_reference<int&&>>);
...
static_assert(!is_base<int, int>);

static_assert(is_constructible<int, int>);
static_assert(is_constructible<int, double>);

static_assert(is_destructible<int>);

static_assert(is_same<int, remove_reference<int>>);
static_assert(is_same<int, remove_reference<int&>>);
static_assert(is_same<int, remove_reference<int&&>>);
...
struct basic_coro {
    struct promise {
        ...
        void return_void() { printf("basic_coro::return_void()\n"); }
