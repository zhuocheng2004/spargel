...
App::App(AppDelegate* delegate) : delegate_{delegate} { delegate_->SetProcTable(&table_); }
...
bool App::Init() {
    ...
    if (!delegate_->SelectInstanceLayers(instance_layers_, selected_instance_layers_)) return false;
    if (!SelectInstanceExtensions()) return false;
    if (!delegate_->SelectInstanceExtensions(instance_extensions_, selected_instance_extensions_))
        return false;
...
#define VULKAN_GENERAL_PROC(name)                                                      \
    table_.name = reinterpret_cast<PFN_##name>(vkGetInstanceProcAddr(nullptr, #name)); \
    if (!table_.name) {                                                                \
        LOG(Error, "cannot load " #name);                                              \
        return false;                                                                  \
    }
#include "vulkan_triangle/vulkan_procs.inc"
...
bool App::EnumerateInstanceLayers() {
    ...
    result = table_.vkEnumerateInstanceLayerProperties(&count, instance_layers_.data());
    instance_layers_.resize(count);
    return true;
}
...
bool App::EnumerateInstanceLayerExtensions(char const* layer) {
    ...
    auto result = table_.vkEnumerateInstanceExtensionProperties(layer, &count, nullptr);
    if (result != VK_SUCCESS) {
        LOG(Error, "cannot enumerate instance extensions");
        return false;
    }
    exts.resize(count);
    result = table_.vkEnumerateInstanceExtensionProperties(layer, &count, exts.data());
    if (result != VK_SUCCESS) {
        LOG(Error, "cannot enumerate instance extensions");
        return false;
    }
    exts.resize(count);
    instance_extensions_.insert(instance_extensions_.end(), exts.begin(), exts.end());
    return true;
}
...
bool App::SelectInstanceLayers() {
    has_validation_ = SelectLayerByName("VK_LAYER_KHRONOS_validation", instance_layers_,
...
void App::PrintSelectedInstanceLayers() {
    LOG(Info, "list of selected instance layers (", selected_instance_layers_.size(), " in total)");
...
void App::PrintSelectedInstanceExtensions() {
    LOG(Info, "list of selected instance extensions (", selected_instance_extensions_.size(),
        " in total)");
...
bool App::SelectInstanceExtensions() {
    has_debug_utils_ = SelectExtensionByName("VK_EXT_debug_utils", instance_extensions_,
                                             selected_instance_extensions_);
    auto has_surface = SelectExtensionByName("VK_KHR_surface", instance_extensions_,
                                             selected_instance_extensions_);
    if (!has_surface) {
        LOG(Error, "cannot find extension VK_KHR_surface");
        return false;
    }
    has_portability_ = SelectExtensionByName("VK_KHR_portability_enumeration", instance_extensions_,
                                             selected_instance_extensions_);
    return true;
}
...
#define VULKAN_INSTANCE_PROC(name)                                                       \
    table_.name = reinterpret_cast<PFN_##name>(vkGetInstanceProcAddr(instance_, #name)); \
    if (!table_.name) {                                                                  \
        Log(Error, "cannot load " #name);                                                \
        return false;                                                                    \
    }
...
bool App::CreateDebugMessenger() {
    ...
    info.pUserData = this;
    auto result =
        table_.vkCreateDebugUtilsMessengerEXT(instance_, &info, nullptr, &debug_messenger_);
    if (result != VK_SUCCESS) {
...
bool App::EnumeratePhysicalDevices() {
    ...
    result = table_.vkEnumeratePhysicalDevices(instance_, &count, physical_devices_.data());
    if (result != VK_SUCCESS) {
        LOG(Error, "cannot enumerate physical devices");
        return false;
    }
...
bool App::QueryDeviceExtensions(size_t idx) {
    ...
    uint32_t count;
    auto result = table_.vkEnumerateDeviceExtensionProperties(adapter, nullptr, &count, nullptr);
    if (result != VK_SUCCESS) {
...
void App::QueryDeviceQueueFamilies(size_t idx) {
    ...
    table_.vkGetPhysicalDeviceQueueFamilyProperties(adapter, &count, prop.queue_families.data());
    prop.queue_families.resize(count);
}
...
bool App::QueryDeviceSurfaceCapabilities(size_t idx) {
    auto adapter = physical_devices_[idx];
    auto& prop = physical_device_properties_[idx];
    auto result = table_.vkGetPhysicalDeviceSurfaceCapabilitiesKHR(adapter, surface_,
                                                                   &prop.surface_capabilities);
    if (result != VK_SUCCESS) {
...
bool App::QueryDeviceSurfaceFormats(size_t idx) {
    ...
    auto& prop = physical_device_properties_[idx];
    uint32_t count;
    auto result = table_.vkGetPhysicalDeviceSurfaceFormatsKHR(adapter, surface_, &count, nullptr);
    if (result != VK_SUCCESS) {
        LOG(Error, "cannot query surface formats");
        return false;
    }
    prop.surface_formats.resize(count);
    result = table_.vkGetPhysicalDeviceSurfaceFormatsKHR(adapter, surface_, &count,
                                                         prop.surface_formats.data());
    if (result != VK_SUCCESS) {
        LOG(Error, "cannot query surface formats");
        return false;
    }
...
bool App::QueryDevicePresentModes(size_t idx) {
    ...
    uint32_t count;
    auto result =
        table_.vkGetPhysicalDeviceSurfacePresentModesKHR(adapter, surface_, &count, nullptr);
    if (result != VK_SUCCESS) {
        LOG(Error, "cannot query present modes");
        return false;
    }
    prop.present_modes.resize(count);
    result = table_.vkGetPhysicalDeviceSurfacePresentModesKHR(adapter, surface_, &count,
                                                              prop.present_modes.data());
    if (result != VK_SUCCESS) {
        LOG(Error, "cannot query present modes");
        return false;
    }
...
bool App::SelectDeviceExtensions() {
    ...
    auto has_swapchain =
        SelectExtensionByName("VK_KHR_swapchain", available, selected_device_extensions_);
    if (!has_swapchain) {
        LOG(Error, "cannot find extension VK_KHR_swapchain");
        return false;
    }
    SelectExtensionByName("VK_KHR_portability_subset", available, selected_device_extensions_);
...
bool App::SelectQueues() {
    auto const& families = physical_device_properties_[physical_device_id_].queue_families;
    // find one queue family that supports all of graphics, compute, transfer, and
    // present.
    auto expected = VK_QUEUE_GRAPHICS_BIT | VK_QUEUE_COMPUTE_BIT | VK_QUEUE_TRANSFER_BIT;
    for (int i = 0; i < families.size(); i++) {
        auto const& prop = families[i];
        bool valid = (prop.queueFlags & expected) != 0;
        VkBool32 present = VK_FALSE;
        table_.vkGetPhysicalDeviceSurfaceSupportKHR(physical_device_, i, surface_, &present);
        if (valid && present) {
            queue_family_id_ = i;
            return true;
        }
...
bool App::CreateDevice() {
    ...
    auto result = table_.vkCreateDevice(physical_device_, &info, nullptr, &device_);
    if (result != VK_SUCCESS) {
...
#define VULKAN_DEVICE_PROC(name)                                                     \
    table_.name = reinterpret_cast<PFN_##name>(vkGetDeviceProcAddr(device_, #name)); \
    if (!table_.name) {                                                              \
        Log(Error, "cannot load " #name);                                            \
        return false;                                                                \
    }
...
void App::GetQueue() { table_.vkGetDeviceQueue(device_, queue_family_id_, 0, &queue_); }

void App::SelectSurfaceFormat() {
    auto const& formats = physical_device_properties_[physical_device_id_].surface_formats;
...
bool App::CreateSwapchain() {
    ...
    auto result = table_.vkCreateSwapchainKHR(device_, &info, nullptr, &swapchain_);
    if (result != VK_SUCCESS) {
...
bool App::GetSwapchainImages() {
    uint32_t count;
    auto result = table_.vkGetSwapchainImagesKHR(device_, swapchain_, &count, nullptr);
    if (result != VK_SUCCESS) {
        LOG(Error, "cannot get swapchain images");
        return false;
    }
    swapchain_images_.resize(count);
    result = table_.vkGetSwapchainImagesKHR(device_, swapchain_, &count, swapchain_images_.data());
    if (result != VK_SUCCESS) {
        LOG(Error, "cannot get swapchain images");
        return false;
    }
...
bool App::CreateSwapchainImageViews() {
    ...
    for (int i = 0; i < swapchain_images_.size(); i++) {
        ...
        auto result =
            table_.vkCreateImageView(device_, &info, nullptr, swapchain_image_views_.data() + i);
        if (result != VK_SUCCESS) {
...
bool App::CreateCommandPool() {
    ...
    auto result = table_.vkCreateCommandPool(device_, &info, nullptr, &command_pool_);
    if (result != VK_SUCCESS) {
...
bool App::CreateCommandBuffers() {
    ...
    auto result = table_.vkAllocateCommandBuffers(device_, &info, frames_.command_buffer);
    if (result != VK_SUCCESS) {
...
bool App::CreateRenderPass() {
    ...
    auto result = table_.vkCreateRenderPass(device_, &info, nullptr, &render_pass_);
    if (result != VK_SUCCESS) {
...
bool App::CreateFramebuffers() {
    ...
    for (int i = 0; i < framebuffers_.size(); i++) {
        ...
        auto result = table_.vkCreateFramebuffer(device_, &info, nullptr, &framebuffers_[i]);
        if (result != VK_SUCCESS) {
...
VkBool32 App::DebugMessageCallback(VkDebugUtilsMessageSeverityFlagBitsEXT severity,
...
void App::DestroySurface() { table_.vkDestroySurfaceKHR(instance_, surface_, nullptr); }

void App::DestroyDevice() { table_.vkDestroyDevice(device_, nullptr); }

void App::DestroySwapchain() { table_.vkDestroySwapchainKHR(device_, swapchain_, nullptr); }
...
void App::DestroyCommandPool() { table_.vkDestroyCommandPool(device_, command_pool_, nullptr); }
...
void App::DestroyRenderPass() { table_.vkDestroyRenderPass(device_, render_pass_, nullptr); }
...
bool App::WaitAndResetFence() {
    VkResult result;
    result = table_.vkWaitForFences(device_, 1, frames_.render_fence + current_frame_, VK_TRUE,
                                    UINT64_MAX);
    if (result != VK_SUCCESS) {
        LOG(Error, "cannot wait for fence");
        return false;
    }
    result = table_.vkResetFences(device_, 1, frames_.render_fence + current_frame_);
    if (result != VK_SUCCESS) {
...
bool App::AcquireNextImage(uint32_t* id) {
    auto result = table_.vkAcquireNextImageKHR(
        device_, swapchain_, UINT64_MAX, frames_.swapchain_semaphore[current_frame_], nullptr, id);
...
bool App::EndCommandBuffer() {
    auto result = table_.vkEndCommandBuffer(frames_.command_buffer[current_frame_]);
    if (result != VK_SUCCESS) {
...
bool App::Submit() {
    ...
    auto result = table_.vkQueueSubmit(queue_, 1, &info, frames_.render_fence[current_frame_]);
    if (result != VK_SUCCESS) {
...
void App::BeginRenderPass(VkCommandBuffer cmd, uint32_t image_id, VkClearColorValue clear_color) {
    VkClearValue clear;
...
void App::render(void* p) {
    auto self = (App*)p;

    if (self->closing_) return;

    if (!self->WaitAndResetFence()) return;

    uint32_t swapchain_image_id;
    if (!self->AcquireNextImage(&swapchain_image_id)) return;
    if (!self->BeginCommandBuffer()) return;

    auto cmd = self->frames_.command_buffer[self->current_frame_];
    ...
    float flash = abs(sin(self->frame_count_ / 120.f));
    VkClearColorValue clearValue = {{abs(sin(self->frame_count_ / 120.f + 72)),
                                     abs(sin(self->frame_count_ / 120.f + 36)), flash, 1.0f}};
    ...
    self->BeginRenderPass(cmd, swapchain_image_id, clearValue);
    ...
    self->table_.vkCmdEndRenderPass(cmd);

    if (!self->EndCommandBuffer()) return;
    if (!self->Submit()) return;
    if (!self->Present(swapchain_image_id)) return;

    self->current_frame_ = (self->current_frame_ + 1) % NumFramesInFlight;
    self->frame_count_++;
}
