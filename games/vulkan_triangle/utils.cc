...
bool SelectLayerByName(char const* layer, std::vector<VkLayerProperties> const& available,
                       std::vector<char const*>& selected) {
    auto iter = std::find_if(available.begin(), available.end(),
...
bool SelectExtensionByName(char const* extension,
                           ...
                           std::vector<char const*>& selected) {
    auto iter = std::find_if(available.begin(), available.end(), [=](auto const& prop) {
